	...
	float check_output[nImg][nOfm][ofhp][ofwp])
{
	...
	if (version == 2) {
		...
		l_start = libxsmm_timer_tick();
		for (i = 0; i < iters; i++) {
			padded_conv_relu_fp_libxsmm_core_gemm_fused3(nImg, nIfm, nOfm, ifhp, ifwp, ofhp, ofwp, ifh, ifw,
				ofh, ofw, pad_h, pad_w, pad_h_in, pad_w_in, pad_h_out,
				pad_w_out, kh, kw, stride_h, stride_w, pad_gemm_input, output, filter, iters);
		}
		...
		l_start = libxsmm_timer_tick();
		for (i = 0; i < iters; i++) {
			padded_conv_relu_fp_libxsmm_core_gemm_fused4(nImg, nIfm, nOfm, ifhp, ifwp, ofhp, ofwp, ifh, ifw,
				ofh, ofw, pad_h, pad_w, pad_h_in, pad_w_in, pad_h_out,
				pad_w_out, kh, kw, stride_h, stride_w, pad_gemm_input, output, filter, iters);
		}
